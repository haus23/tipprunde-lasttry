...
const sidebarLinks: {
  ...
  },
  {
    ...
    icon: CalendarIcon,
    label: 'Spiele',
    visible: (championship, rounds) => championship !== null && rounds?.length > 0
  }
];
    ...
    <div className="flex grow flex-col border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <div className="flex grow flex-col overflow-y-auto">
        ...
        <nav className="mt-2 grow space-y-1 px-2">
          {sidebarLinks.filter(link => link.visible(championship, rounds)).map((link, ix) =>
            <SidebarLink key={ix} to={link.to} icon={link.icon}>{link.label}</SidebarLink>)
          }
        </nav>
      </div>
          ...
          <div className="flex items-center">
            <div>
              {user.photoURL ? (
                ...
                <UserIcon
                  className="inline-block h-9 w-9 rounded-full bg-gray-200 p-1 text-gray-400 dark:bg-gray-900 dark:text-gray-300"/>
              )}
            </div>
